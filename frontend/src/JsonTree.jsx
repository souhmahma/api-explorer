...
function JsonNode({ data, depth = 0, keyName = null }) {
  ...
  if (count === 0) {
    return (
      <span className="jn-empty">
        {keyName !== null && <><span className="jn-key">"{keyName}"</span><span className="jn-colon">: </span></>}
        {open}{close}
      </span>
    )
  }
  ...
  return (
    <div className="jn-block" style={{ marginLeft: depth > 0 ? indent : 0 }}>
      ...
      {!collapsed && (
        <div className="jn-children">
          {entries.map(([k, v], i) => (
            <div key={k} className="jn-entry">
              {typeof v === "object" && v !== null ? (
                <JsonNode data={v} depth={depth + 1} keyName={isArray ? null : k} />
              ) : (
                <span>
                  {!isArray && <><span className="jn-key">"{k}"</span><span className="jn-colon">: </span></>}
          ...
          <div className="jn-close"><span className="jn-bracket">{close}</span></div>
        </div>
      )}
    </div>
  )
